...
| G3 | **No penalty left standing** | When a device is recovered, the penalty charged for losing or writing it off is always reversed — never partly, never silently. | R9a · R10a · AC-GRD-3 · MQ-4 |
| G4 | **Never more than was taken** | A reversal returns at most what was charged, and never runs twice for the same penalty. | R9 MUST NOT(b) · R8 MUST NOT(a) · AC-DUP-2 · AC-GRD-4 · MQ-4 |
...
| R10 | As a CSP, my device was written off (WRITTEN_OFF), and I want the charge cleared the moment I produce it. | **(a)** Clear the write-off liability immediately on confirmation. | Hold the clearance to a cycle — write-off is not cycle-bound (§5 C-02 note). |
...
| T11 | Awaiting confirmation | CSP confirms | Source state = WRITTEN_OFF | Confirmed | Device to IDLE (G2). Connection closed out as in T8 where one is still open. Write-off liability cleared immediately, with no cycle wait (R10a, G3). The CSP's device exposure is restored. Idle transition announced to the customer system (R11a, G5). Confirmation recorded (R12a). |
...
| MQ-4 | For every reversal: whether it was cancelled before charge or credited back, the amount, which pocket it landed in, whether it ever exceeded the original charge, and whether any penalty was reversed twice. | G3 · G4 invariant |
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-WOFF-1 | **Given** device GX78440 was WRITTEN_OFF under CSP a0a0j6 carrying a ₹3,200 liability, **When** a0a0j6 confirms custody on 3 Aug 2026 at 14:00, **Then** GX78440 is IDLE, the ₹3,200 liability is cleared the same moment, and the CSP's device exposure is restored — no cycle is waited for. | R10a · R10 MUST NOT · T11 · G2 · G3 | Settled |
...
| AC-GRD-3 | **Given** a LOST device and a WRITTEN_OFF device, both carrying a charge, **When** each is confirmed, **Then** each charge is fully reversed — no partial reversal and no case where the device is recovered but the charge stands. | G3 · T10 · T11 | Settled |
...
| Written off — `WRITTEN_OFF` | The device has been closed out as a total loss with a liability charged to the CSP, covering the device's value and accumulated carrying charges. | Device custody |
...
| Clear a write-off liability immediately and restore the CSP's exposure, outside any cycle. | R10a · T11 · G3 |
